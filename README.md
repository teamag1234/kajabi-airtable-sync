...
- **Altas a mano en Kajabi** (People, Grant offer, sin compra): no generan compra en la API, así que el importador revisa además, cada día, las ofertas que cada cliente tiene concedidas. Un acceso de curso nuevo que no esté en Renovaciones entra con fecha de inicio = día de detección (anotado en Notas como fecha estimada). Los pares cliente|oferta ya vistos se guardan en la tabla **Altas manuales Kajabi** (Importado o Ignorado) para no repetirlos; la primera pasada (`node scripts/seed-altas-manuales.mjs`) apuntó como Ignorado el histórico de accesos antiguos sin compra.
