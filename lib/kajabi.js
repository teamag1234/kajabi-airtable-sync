...
export async function kajabiGet(url, params) {
...
/** POST/PATCH con cuerpo JSON:API. Reintenta ante 429 como kajabiGet. */
export async function kajabiSend(method, url, data) {
  const token = await getKajabiAccessToken();
  for (let intento = 0; intento < 4; intento++) {
    try {
      const response = await axios({
        ...
        headers: {
          ...
          'Content-Type': 'application/vnd.api+json', 'User-Agent': USER_AGENT,
        },
      });
      return response.data;
    } catch (error) {
      if (error.response?.status === 429 && intento < 3) {
        await new Promise((r) => setTimeout(r, 1500 * (intento + 1)));
        continue;
      }
      throw error;
