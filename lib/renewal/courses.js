...
export const CURSOS = [
  ...
  { key: 'directo', familia: 'directo', nombre: 'Directo al Aptis', meses: 12, ofertasKajabi: ['Directo al Aptis', 'Directos al Aptis', 'Aptis Expert', 'MEGA OFERTA SOLO PARA TI', 'ENGLISH SUMMER CHANCE'], renovaciones: RENOVACIONES_DIRECTO },
...
/**
 * Productos de Kajabi (ids) que identifican cada familia. Cualquier oferta que
 * dé acceso a uno de ellos es un alumno de esa familia aunque el nombre de la
 * oferta no coincida con nada (ofertas gratuitas para pago por transferencia o
 * efectivo, packs, ofertas especiales...). Si el nombre no dice la variante se
 * usa el curso por defecto de la familia.
 */
export const PRODUCTOS_KAJABI = {
  '2149043351': 'directo', // DIRECTO AL APTIS ⭐
  '2147864433': 'directo', // APTIS EXPERT (retirado, equivale a Directo al Aptis)
  '2148749393': 'directo', // APTIS EXPERT 🚀
  '2147557314': 'accelerator', // APTIS ACCELERATOR
  '2149172408': 'level', // APTIS LEVEL 🔥 New
  '2148852898': 'level', // APTIS LEVEL 🔥
  '2148844083': 'level', // APTIS LEVEL
  '2148889472': 'ten-tu-aptis', // TEN TU APTIS
};

const CURSO_POR_DEFECTO = { directo: 'directo', accelerator: 'accelerator', level: 'level', 'ten-tu-aptis': 'ten-tu-aptis-4m' };

/**
 * Busca el curso de una oferta de Kajabi usando todo lo que sabemos de ella:
 * título, título interno y productos a los que da acceso. El nombre manda
 * (distingue Express, Tutorizado...); si no coincide, decide el producto.
 */
export function buscarCursoPorOfertaKajabi({ titulo, interno, productos } = {}) {
  const porNombre = buscarCursoPorOferta(titulo) || buscarCursoPorOferta(interno);
  if (porNombre) return porNombre;
  for (const id of productos || []) {
    const familia = PRODUCTOS_KAJABI[String(id)];
    if (familia) return buscarCurso(CURSO_POR_DEFECTO[familia]);
  }
  return null;
}
 ...
 */
const NO_ES_CURSO = ['reserva de clase', 'reserva de clases', 'cambio tarjeta', 'cambio de tarjeta', 'taller', 'sesion individual', 'sesiones individuales'];

export function buscarCursoPorOferta(nombreOferta) {
  const n = normalizar(nombreOferta);
  if (NO_ES_CURSO.some((x) => n.includes(x))) return null;
  let mejor = null;
  let mejorPuntos = 0;
