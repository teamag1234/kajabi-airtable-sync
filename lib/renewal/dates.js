...
/**
 * Dadas fecha de inicio y duración devuelve fin de acceso y fecha límite de
 * renovación (7 días después del fin). La duración puede ser un número de
 * meses o un objeto { meses } / { dias }.
 */
export function calcularFechas(fechaInicio, duracion) {
  const d = typeof duracion === 'number' ? { meses: duracion } : duracion || {};
  if (!d.meses && !d.dias) throw new Error('Falta la duración del acceso (meses o días)');
  const fechaFin = d.dias ? sumarDias(fechaInicio, d.dias) : sumarMeses(fechaInicio, d.meses);
