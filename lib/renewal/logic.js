...
import { buscarCurso, buscarCursoPorOferta, ofertasRenovacionDeCurso, duracionCurso } from './courses.js';
...
import { urlAprobado } from './links.js';
 ...
 */
export async function altaRenovacion({ email, nombre, curso, fechaInicio, fechaFin, meses, dias, origen = 'manual', notas }) {
 ...
 */
export async function procesarSecuencia({ dryRun = false, hoy = hoyMadrid(), testTo = process.env.RENEWAL_TEST_TO } = {}) {
  ...
  for (const fila of filas) {
    ...
    try {
      ...
      const ctx = {
        ...
        ofertas: ofertasRenovacionDeCurso(cursoDeLaFila),
