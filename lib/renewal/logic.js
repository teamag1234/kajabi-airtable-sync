...
import { buscarCurso, buscarCursoPorOferta, getOfertasRenovacion } from './courses.js';
 ...
 * Si no se reconoce el curso pero llegan `meses`, se acepta igualmente.
 */
export async function altaRenovacion({ email, nombre, curso, fechaInicio, meses, origen = 'manual' }) {
  ...
  const mesesAcceso = Number(meses || (cursoCatalogo && cursoCatalogo.meses));
  if (!cursoNombre) throw new Error('Falta el curso');
  if (!mesesAcceso) throw new Error(`Curso no reconocido y sin meses de acceso: ${curso}`);

  const fechas = calcularFechas(fechaInicio, mesesAcceso);
  ...
  for (const fila of filas) {
    ...
    try {
      ...
      if (!fechaFin) {
        ...
        const meses = Number(f[F.meses] || (cursoCatalogo && cursoCatalogo.meses));
        if (!f[F.inicio] || !meses) {
          resumen.detalle.push({ email, curso: f[F.curso], accion: 'omitido', motivo: 'faltan fecha de inicio o meses' });
          continue;
        }
        const fechas = calcularFechas(f[F.inicio], meses);
