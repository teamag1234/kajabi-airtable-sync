...
import { detectarAltasManuales } from './grants.js';
 ...
 */
export async function importarDesdeKajabi({ desde, incremental = false, dryRun = false, hoy = hoyMadrid(), aprobados = null, altasManuales = false, log = console.log } = {}) {
  ...
  if (altasManuales) {
    try {
      resumen.altasManuales = await detectarAltasManuales({ dryRun, hoy, log });
      resumen.altasManuales.detalle = resumen.altasManuales.detalle.filter((d) => d.accion !== 'ignorado');
    } catch (error) {
      console.error('Error detectando altas manuales:', error.message);
      resumen.altasManuales = { error: error.message };
    }
  }
  return resumen;
}
