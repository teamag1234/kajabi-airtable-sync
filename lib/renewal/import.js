...
import { buscarCursoPorOferta, buscarRenovacionPorOferta, duracionCurso } from './courses.js';
 ...
 *
 * Reglas:
 *  - La oferta (título o título interno) tiene que coincidir con un curso del
 *    catálogo, directamente o como oferta de renovación de ese curso.
...
export function mapearCompra({ purchase, offer, customer, hoy = hoyMadrid() }) {
  ...
  const curso = buscarCursoPorOferta(titulo) || buscarCursoPorOferta(interno);
...
/** Agrupa accesos por alumno y curso quedándose con el que termina más tarde. */
export function consolidarAccesos(accesos) {
  const porClave = new Map();
  for (const acc of accesos) {
    const clave = `${acc.email}|${acc.cursoKey}`;
...
export async function importarDesdeKajabi({ desde, incremental = false, dryRun = false, hoy = hoyMadrid(), aprobados = null, log = console.log } = {}) {
  ...
  for (const acc of consolidados) {
    ...
    try {
      const r = await altaRenovacion({ email: acc.email, nombre: acc.nombre, curso: acc.cursoKey, fechaInicio: acc.fechaInicio, fechaFin: acc.fechaFin, origen: 'kajabi-import', notas });
      ...
      resumen.detalle.push({ email: acc.email, curso: acc.curso, fechaFin: acc.fechaFin, accion: r.accion });
