...
const ABREVIATURAS_MARIA = /^(m|m\.|mª|m\.ª|ma\.|mᵃ|ma)$/i;
const INICIAL = /^[a-záéíóúñ]\.?$/i;
const PARTICULA = /^(de|del|la|las|los|el|y|da|do|dos|van|von|di)$/i;

function capitalizar(token) {
  if (token.length <= 2) return token; // "JJ", "Di": no sabemos si es inicial o nombre
  if (token === token.toUpperCase() || token === token.toLowerCase()) {
    return token.split('-').map((t) => t.charAt(0).toUpperCase() + t.slice(1).toLowerCase()).join('-');
  }
  return token;
}

/**
 * Nombre de pila para el saludo. Salta iniciales y abreviaturas de María
 * ("M. Inmaculada" → "Inmaculada", "J. Ismael" → "Ismael"), arregla las
 * mayúsculas ("BELEN REYES" → "Belen") y devuelve '' si no hay nada fiable
 * (vacío, un email, o un nombre de una o dos letras), para saludar sin nombre.
 */
export function primerNombre(nombre) {
  ...
  for (const t of tokens) {
    if (ABREVIATURAS_MARIA.test(t) || INICIAL.test(t) || PARTICULA.test(t)) continue;
    candidato = t.replace(/[.,;:]+$/g, '');
    break;
  }
  if (!candidato) {
    // solo había iniciales: si alguna era de María, saludamos así
    if (tokens.some((t) => ABREVIATURAS_MARIA.test(t))) return 'María';
    return '';
  }
  candidato = capitalizar(candidato);
  return candidato.length >= 3 ? candidato : '';
}
...
export function generarEmail(paso, ctx) {
  ...
  const conseguido = ctx.urlAprobado ? `¿Ya tienes el APTIS? Dínoslo aquí y dejamos de escribirte: ${ctx.urlAprobado}` : '';
  const texto = [saludo, ...parrafos.map((p) => (p === '{{OFERTAS}}' ? ofertasTexto(ctx.ofertas) : p)), firma, conseguido].filter(Boolean).join('\n\n');
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;">
  <tr>
    <td style="background:#0a0e27;padding:18px 28px;" align="center">
      <img src="${escapeHtml(ENLACES.logo)}" alt="AG Academy" width="150" style="display:block;width:150px;max-width:60%;height:auto;border:0;">
    </td>
  </tr>
  <tr>
    ...
    <td style="padding:8px 28px 8px 28px;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-top:1px solid #e5e7eb;">
        <tr><td style="padding:18px 0 6px 0;">
          <p style="margin:0 0 10px 0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:14px;line-height:1.5;color:#1f2937;">¿Ya tienes el APTIS? Pulsa aquí y dejamos de escribirte. Y de paso nos cuentas qué te ha parecido.</p>
          <a href="${escapeHtml(ctx.urlAprobado)}" style="display:inline-block;background:#ffffff;color:#0a0e27;text-decoration:none;font-weight:600;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:14px;padding:11px 16px;border-radius:8px;border:2px solid #0a0e27;">✅ Ya lo he conseguido</a>
        </td></tr>
      </table>
    </td>
  </tr>` : ''}
